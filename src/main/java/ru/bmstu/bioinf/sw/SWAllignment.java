...
public class SWAllignment {
    ...
    private static final Integer borderAdd = 32;
    ...
    public SWAllignment(String searched, String fromBase,
        ...
        Integer top = borders.getKey();
        Integer bottom = borders.getValue();
        Integer left = routeStart.getDataSetSeqCoordinate() + routeStart.getSearchedSeqCoordinate();
        Integer right = routeFinish.getDataSetSeqCoordinate() + routeFinish.getSearchedSeqCoordinate();

        computeMatrix(top, bottom, left, right);
    }

    private void computeMatrix(Integer top, Integer bottom, Integer left, Integer right) {
        matrix = new LinkedHashMap<>();
        maxNode = new SWNode(0,0, 0.0f, SWNode.getZeroNode());

        Integer searchStartCoord = Integer.max((left - top) / 2, 0);
        Integer searchUntilCoord = Integer.min((right - bottom) / 2 + 1, searched.length());

        for(Integer i = searchStartCoord; i < searchUntilCoord; ++i) {
            Integer start = Integer.max(
                    Integer.max(left - i, bottom + i),
                    0);
            Integer until = Integer.min(
                    Integer.min(right - i + 1, top + i + 1),
                    fromBase.length());
            for(Integer j = start; j < until; ++j) {
                SWNode nodeij = SWNode.max(
                        SWNode.getZeroNode(),
                        new SWNode(i, j, (float) table.getE(),
                                matrix.getOrDefault(
                                        new Pair<>(i - 1, j),
                                        new SWNode(
...
class SWNode {
    public Integer getSearchedCoord() {
    ...
    private Integer searchedCoord;
    private Integer baseCoord;
    private SWNode parent;
    private Float score;
    private static SWNode zero = null;

    public SWNode(Integer searchedCoord, Integer baseCoord, Float addition, SWNode parent) {
